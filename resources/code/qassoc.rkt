...
(define (remove-unentangled-qubit reg) ; removes the last qubit from the system (assumes it is unentangled)
    (let [(new-reg (submatrix reg 1 (build-list (/ (matrix-num-cols reg) 2) (lambda (x) (* x 2)))))]
...
(define (Grover-part tau patterns)
    (letrec ([qubits (length tau)]
        ...
        [T (calc-T (length patterns) (- (expt 2 (count (lambda (x) (eq? x '?)) tau)) 1) 1 N)] ; Assume that there is only one match in the learned patterns
        [big-control-Z (constr-big-control-Z N)])
