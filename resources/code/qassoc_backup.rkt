...
(define (Grover-part tau patterns)
    (letrec ([qubits (length tau)]
        ...
        [T 0]; (calc-T (length patterns) (- (expt 2 (count (lambda (x) (eq? x '?)) tau)) 1) 1 N)] ; Assume that there is only one match in the learned patterns
        [big-control-Z (diagonal-matrix (build-list (expt 2 (+ 0 qubits)) (lambda (x) (if (= x 0) -1 1))))])
        ...
        ;(Hadamard (apply-gate (Hadamard register) '(0 1 2 3) big-control-Z)) ; One Grover diffusion
      
      (Hadamard register)
        (displayln register)
        (displayln "")
      (apply-gate register all-qubits big-control-Z)
        (displayln register)
        (displayln "")
      (Hadamard register)
